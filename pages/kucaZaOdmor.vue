  ...
  <div>
    ...
    <v-form ref="form" v-model="valid" lazy-validation>
      <v-text-field
        ...
        required
      ></v-text-field>
      <v-text-field v-model="obrazac.prebivaliste" :rules="nameRules" label="Prebivalište" required></v-text-field>
      <v-text-field v-model="obrazac.oib" :rules="[oibID]" label="OIB" required></v-text-field>

      <v-text-field v-model="obrazac.email" :rules="emailRules" label="E-mail" required></v-text-field>

      <p>Prijavljujem kuću za odmor (zgrada/dio zgrade/stan) koju koristim povremeno ili sezonski, na adresi:</p>
      <v-text-field
      ...
      <v-text-field v-model="obrazac.kvadratura" :rules="nameRules" label="u m2" required></v-text-field>

      <p>Vlasnik sam kuće za odmor od:</p>
      <v-text-field v-model="obrazac.datum" :rules="nameRules" label="datum" required></v-text-field>

      <p>Podaci se daju u svrhu razreza poreza na kuće za odmor za</p>
      <v-text-field v-model="obrazac.godina" :rules="nameRules" label="godinu" required></v-text-field>
      <p>Rješenje o porezu na kuću za odmor dostavljat će se:</p>
      <v-radio-group v-model="obrazac.rijesenje">
        <v-radio label="poštom na adresu prebivališta/sjedišta vlasnika"></v-radio>
      ...
      <v-btn @click="fillData()">Submit</v-btn>
    </v-form>
  </div>
</template>
